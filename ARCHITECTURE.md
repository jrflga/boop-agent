...
│                      EXPRESS + WS SERVER                        │
│                                                                 │
│   POST /telegram/webhook   ──────►  Interaction Agent           │
...
- Replies stream through Telegram `sendMessage` (markdown stripped, chunked to fit Telegram limits).
...
- The result is written as an `automationRun` row, and (if `notifyConversationId` points at a `telegram:<chat_id>` conversation) pushed back out via Telegram.
...
| `webhookDedup` | Provider-neutral webhook dedup | provider, handle, claimedAt |
...
2.  telegram.ts:  verify secret, authorize chat/user, dedup update_id, spawn handleUserMessage()
